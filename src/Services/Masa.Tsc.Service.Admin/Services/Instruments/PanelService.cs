...
namespace Masa.Tsc.Service.Admin.Services.Instruments
{
    public class PanelService : ServiceBase
    {
        ...
        private async Task AddAsync([FromServices] IEventBus eventBus, [FromBody] AddPanelDto model)
        {
            await eventBus.PublishAsync(new AddPanelCommand(model));
        }

        private async Task UpdateAsync([FromServices] IEventBus eventBus, [FromBody] UpdatePanelDto model)
        {
