    ...
    internal override async Task LoadAsync(ProjectAppSearchModel query)
    {
        ...
        _data = await ApiCaller.MetricService.GetMultiRangeAsync(new RequestMultiQueryRangeDto
        {
            MetricNames = new List<string> {
                $"round(histogram_quantile(0.50,sum(increase(http_server_duration_bucket[{MetricConstants.TimePeriod}])) by (le)),0.01)",
                $"round(histogram_quantile(0.75,sum(increase(http_server_duration_bucket[{MetricConstants.TimePeriod}])) by (le)),0.01)",
                $"round(histogram_quantile(0.90,sum(increase(http_server_duration_bucket[{MetricConstants.TimePeriod}])) by (le)),0.01)",
                $"round(histogram_quantile(0.95,sum(increase(http_server_duration_bucket[{MetricConstants.TimePeriod}])) by (le)),0.01)",
                $"round(histogram_quantile(0.99,sum(increase(http_server_duration_bucket[{MetricConstants.TimePeriod}])) by (le)),0.01)"
            },
            Start = StartTime,
            Service = query.AppId,
            End = EndTime,
