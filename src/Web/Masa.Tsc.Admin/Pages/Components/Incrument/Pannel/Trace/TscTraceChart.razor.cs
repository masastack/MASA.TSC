...
public partial class TscTraceChart
{
    ...
    //{
    //    var data = await ApiCaller.TraceService.GetAggegationAsync(new RequestAggregationDto
    //    {
    //        End = Query.End,
    //        Start = Query.Start,
    ...
    public async Task LoadAsync()
    {
        ...
        var data2 = await ApiCaller.TraceService.GetAggegationAsync(new RequestAggregationDto
        {
            End = Query.End,
            Start = Query.Start,
            FieldMaps = new RequestFieldAggregationDto[] {
        ...
        List<ChartViewDto> list = new()
        {
            new ChartViewDto
            {
                Title = "Span Count",
                ChartType = "bar",
                Pointes = data.Data.Select(item => item.Y).ToArray(),
            },
            new ChartViewDto
            {
                Title = "Trace Count",
                ChartType = "bar",
                Pointes = data2.Data.Select(item => item.Y).ToArray(),
            }
        };
