...
public class TraceDtoConverter : JsonConverter<TraceDto>
{
    public override TraceDto? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        if (JsonDocument.TryParseValue(ref reader, out var doc))
        {
            var jsonObject = doc.RootElement;
            var rootText = jsonObject.GetRawText();
            var result = JsonSerializer.Deserialize<TraceDto>(rootText);
            if (result == null)
                return default;
    ...
    public override void Write(Utf8JsonWriter writer, TraceDto value, JsonSerializerOptions options)
    {
